    ...
    for r_start, c_start in anchors:
        ...
        for offset in range(1, 10): 
            ...
            if is_valid_device and d["容量"] > 0:
                ...
                # 鐵損對照表 (單位：kW)
                   IRON_LOSS_MAP = {
                   10: 0.118, 15: 0.16, 25: 0.261, 37.5: 0.359, 50: 0.32, 75: 0.45,
                   100: 0.5, 150: 0.9, 200: 1.15, 300: 1.57, 400: 1.99, 500: 2.36,
                   600: 2.75, 750: 3.34, 1000: 4.2, 1250: 5.25, 1500: 5.0, 2000: 6.3,
                   2500: 7.36, 3000: 8.83
                    }
                   if d["容量"] > 0:
                    # --- 更新後的鐵損計算 (查表優先，否則 * 3) ---
                    if d["容量"] in IRON_LOSS_MAP:
                        # 查表得到的是 kW，轉換成 W 方便後續統一計算
                        d["鐵損"] = IRON_LOSS_MAP[d["容量"]] * 1000
                    else:
                        # 表格沒出現的數字，直接 * 3 (W)
