...
if excel_file:
    ...
    for sheet_name, raw_df in all_sheets.items():
        ...
        for r_start, c_start in anchors:
            for offset in range(1, 15):
                ...
                if d["容量"] > 0:
                    # --- 3. 數據自動計算邏輯 (核心：不抓取，直接算) ---
                    # A. 鐵損估算 (Wi) = 容量 * 2 (W)
                    d["鐵損"] = d["容量"] * 2.0
                    # B. 滿載銅損 (Wc_full) = 容量 * 12 (W)
                    full_copper = d["容量"] * 12.0
                    # C. 實際銅損 = 滿載銅損 * (負載率/100)^2
                    d["實際銅損"] = full_copper * ((d["負載率"] / 100) ** 2)
                    # D. 改善前耗能 = (鐵損 + 實際銅損) * 8760 / 1000
    ...
    if all_transformer_data:
        ...
        add_row("總裝置容量", f"{total_cap:,.0f} kVA")
        add_row("設備規格分布", "、".join([f"{k}kVA x {v}台" for k, v in sorted(cap_counts.items(), reverse=True)]))
        add_row("平均負載利用率", f"{avg_usage:.2f} %")
        doc.add_page_break()
        ...
        doc.save(output)
        output.seek(0)
        st.download_button("📥 下載完整分析報告", output, "Final_Report.docx")
