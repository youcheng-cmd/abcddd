...
if excel_file:
    ...
    for sheet_name, raw_df in all_sheets.items():
        ...
        for r_start, c_start in anchors:
            for offset in range(1, 15): # 稍微擴大掃描寬度
                ...
                for r_offset in range(0, 55): # 稍微擴大垂直掃描
                    ...
                    # 數據精確抓取
                    if any(k in label for k in ["位置", "建築"]): d["建築物"] = val_str
                    ...
                    if any(k in label for k in ["負載損", "銅損", "Wc"]):
    ...
    if all_transformer_data:
        ...
        cap_dist = "、".join([f"{k:,.0f}kVA x {v}台" for k, v in sorted(cap_counts.items(), reverse=True)])
        ...
        for item in all_transformer_data:
            ...
            row_data = [
                d["建築物"], d["編號"], d["年份"], d["廠牌"], f"{d['容量']:.0f}", d["型式"], 
