...
if excel_file:
        ...
        for r_start, c_start in anchors:
            for offset in range(1, 12):
                ...
                d = {"建築物": "-", "編號": "-", "年份": 0, "廠牌": "-", "容量": 0.0, 
                     "型式": "-", "負載率": 0.0, "鐵損": 0.0, "滿載銅損": 0.0, "現況功因": 0.0}
                specs = []
                ...
                for r_offset in range(0, 60):
                   ...
                   # 找到你原本寫功因的地方，改成這樣：
                   if any(k in label_pure for k in ["功因", "功率因數", "PF", "P.F"]):
                       n = extract_number(val)
                       if n > 0:
    ...
    if all_transformer_data:
        ...
        add_sum("設備規格分布", "、".join([f"{k}kVAx{v}台" for k, v in sorted(cap_dist.items(), reverse=True)]))
