    ...
    for sheet_name, raw_df in all_sheets.items():
        ...
        for r in range(len(raw_df)):
            for c in range(len(raw_df.columns)):
                if str(raw_df.iloc[r, c]).replace(' ', '') == "序號":
                    # 檢查下方標籤確保是規格表
                    if r + 1 < len(raw_df):
                        check = str(raw_df.iloc[r+1, c]).replace(' ', '')
                        if any(k in check for k in ["建築", "編號", "位置"]):
                            anchors.append((r, c))
        ...
        for r_start, c_start in anchors:
            ...
            for offset in range(1, 15):
                ...
                # 序號格子如果是空的就跳過
                sn_raw = str(raw_df.iloc[r_start, target_col]).strip()
                if sn_raw in ["nan", ""]: continue
                
                # 初始化設備數據
                d = {"建築物": "-", "編號": sn_raw, "年份": 0, "廠牌": "-", "容量": 0.0, 
                ...
                for r_offset in range(0, 55):
                    ...
                    if r_offset > 0 and lp == "序號": break # 碰到下一組序號就停止
                    
                    val = str(raw_df.iloc[curr_r, target_col]).strip()
                    if lp == "nan" or not lp: continue

                    # 抓取邏輯
                    if any(k in lp for k in ["建築", "位置"]): d["建築物"] = val
                    if "編號" in lp: d["編號"] = val
                    ...
                    if lp == "功因" or any(k in lp for k in ["功率因數", "PF", "P.F"]):
                        ...
                        if n_pf > 0:
                            d["現況功因"] = n_pf / 100 if n_pf > 1 else n_pf

                    specs.append((label, val))
                
                # 確認容量大於 0 才計入，且排除重複編號
                if d["容量"] > 0:
                    dev_id = f"{sheet_name}_{d['編號']}_{d['容量']}"
                    if dev_id in seen_device_keys: continue
