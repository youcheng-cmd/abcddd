...
if excel_file:
    ...
    for sheet_name, raw_df in all_sheets.items():
        ...
        for r_start, c_start in anchors:
            for offset in range(1, 15):
                ...
                d = {"建築物": "-", "編號": sn_val, "年份": 0, "廠牌": "-", "容量": 0.0, 
                ...
                if d["容量"] > 0:
                    ...
                    # --- 公式計算區 ---
                    # 1. 實際銅損 = 滿載銅損 * (負載率/100)^2
                    d["實際銅損"] = d["滿載銅損"] * ((d["負載率"] / 100) ** 2)
                    # 2. 改善前耗能 = (鐵損 + 實際銅損) * 8760 / 1000
    ...
    if all_transformer_data:
        # --- 2. 網頁即時統計摘要 (介面顯示) ---
        total_capacity = sum(t["analysis"]["容量"] for t in all_transformer_data)
        cap_counts = Counter(t["analysis"]["容量"] for t in all_transformer_data)
        usages = [t["analysis"]["負載率"] for t in all_transformer_data if t["analysis"]["負載率"] > 0]
        avg_usage = sum(usages) / len(usages) if usages else 0
        
        st.success(f"✅ 解析完成！符合條件共 {len(all_transformer_data)} 台")
        col1, col2 = st.columns(2)
        with col1:
            st.metric("1. 總裝置容量", f"{total_capacity:,.0f} kVA")
            st.write("**2. 規格台數分布：**")
        ...
        for item in all_transformer_data:
            d_ana = item["analysis"]
            specs = item["specs"]
            doc.add_paragraph().add_run(f"設備詳細資料 (編號：{d_ana['編號']})").bold = True
        ...
        doc.save(output)
        output.seek(0)
        st.download_button("📥 下載完整分析報告", output, f"Transformer_Report_{base_year}.docx")
